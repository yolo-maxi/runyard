      ...
      <Sequence>
        ...
        <Task id="baseline" output={outputs.baseline} retries={0}>
          {async () => {
            ...
            const startHead = execFileSync("git", ["rev-parse", "HEAD"], { cwd: REPO, encoding: "utf8" }).trim();
            return { startHead };
          }}
        </Task>
        ...
        {impl && (
          <Task id="test" output={outputs.test} retries={0}>
            {async () => {
              ...
              let passed = false;
              try {
                out = execFileSync("pnpm", ["test"], { cwd: REPO, encoding: "utf8", maxBuffer: 1024 * 1024 * 64 });
                passed = true;
              } catch (e) {
        ...
        {commit && (
          <Task id="push" output={outputs.push} retries={1}>
            {async () => {
              ...
              let detail = "";
              try {
                detail = execFileSync("git", ["push", "origin", `HEAD:${branch}`], { cwd: REPO, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
              } catch (e) {
        ...
        {push && (
          <Task id="deploy" output={outputs.deploy} retries={0}>
            {async () => {
              ...
              const env = { ...process.env, GIT_SSH_COMMAND: `ssh -i ${DEPLOY_KEY} -o BatchMode=yes -o StrictHostKeyChecking=accept-new` };
              ...
              for (const p of ["/healthz", "/", "/docs", "/app"]) {
                const code = execFileSync("curl", ["-s", "-o", "/dev/null", "-w", "%{http_code}", "--max-time", "15", `${HEALTH_BASE}${p}`], { encoding: "utf8" });
                verify += `${p}:${code} `;
              }
