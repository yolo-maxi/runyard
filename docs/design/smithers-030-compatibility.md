...
| `approve/deny --iteration` (defaults to pending gate's iteration), `--note`, `--by` | **unaffected (gap closed upstream)** | Probed. The known 0.22 iteration>0 fail-closed gap is fixed upstream: `--iteration` now defaults to the pending gate's iteration, so the bridge's existing argv (no explicit iteration) is correct on 0.30; provenance (`--by`, `--note`) continues to ride along. |
...
| `smithers init` (0.30): interactive by default; `--yes --non-interactive` supported; scaffolds pack with own `package.json` (`^0.30.0`) + `node_modules`; **rewrites `agents.ts`** when it carries the `// smithers-source: generated` header (marker-less files are left alone; all other pack files survive re-init customized — verified by marker sweep); installs skills into detected agent homes | **migrate** | Probed (sandboxed `$HOME`). `cliRunnerSetup` passes `--yes --non-interactive`, preserves an existing `agents.ts` transactionally across init (byte-for-byte restore on success, failure, or deletion; crash-recovery for interrupted setups; no backup left behind — regression-tested), and verifies the effective engine version after init. The live workspace's customized `agents.ts` still carries the generated header, so without this it would be clobbered. |
