    ...
    lanes: withPolicy.map((lane) => {
      ...
      return {
        ...
        trigger: {
          ...
          description: "Run product-workflow to propose compact RunYard features and, when enabled, queue isolated review-branch implementations."
        }
      };
    }),
    schedules: [
      {
        ...
        description: "Daily roadmap shaping for RunYard dogfooding. When an operator enables this schedule, product-workflow queues gated implementation runs sequentially in isolated review branches; main changes only through explicit promotion.",
        input: {
          context:
            "Daily RunYard roadmap shaping. Propose and execute a compact set of high-leverage RunYard features through gated implement-change-gated child runs. Each child must use an isolated worktree/review branch and leave main untouched until explicit promotion. For each proposal include rationale, acceptance checks, and a self-contained implementation prompt. Keep the report suitable to paste or summarize into Telegram.",
          competitors: "",
          maxCompetitors: 5,
          maxFeatures: 1,
          agentHarness: "codex",
