...
export const MCP_TOOLS = [
  ...
  { name: "list_ci_repositories", description: "List CI-connected repositories: provider identity, default branch, enablement, and trust policy, plus GitHub App installation state. GitHub stays the canonical repo host; RunYard runs the CI.", inputSchema: { type: "object", properties: { enabled: { type: "boolean", description: "Only repositories enabled for CI." } } } },
  { name: "get_ci_pipeline", description: "Get one CI pipeline by pipeline id or parent run id: trigger provenance (event, delivery, SHAs, PR), trusted config source sha, tested-checkout semantics (head vs merge candidate), the job DAG with live run states, and GitHub check reporting state.", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string" } } } },
  { name: "dispatch_ci_run", description: "Manually dispatch CI for a connected repository (dogfood/recovery path). ref defaults to the default branch and is resolved to an exact sha; config loads from that same trusted revision. Requires an api/admin-scoped token.", inputSchema: { type: "object", required: ["repo"], properties: { repo: { type: "string", description: "Connected repository id or owner/name." }, ref: { type: "string", description: "Branch, tag, or sha to test." } } } },
  ...
  { name: "list_board_definitions", description: "List boards as portable definition summaries: slug, title, lane count, project scope, and whether a transition policy is set. Also returns built-in example slugs (e.g. runyard-development-factory).", inputSchema: { type: "object", properties: {} } },
  { name: "get_example_board_definition", description: "Fetch a built-in example board definition (JSON) — ready to import with import_board_definition. Known slug: runyard-development-factory.", inputSchema: { type: "object", required: ["slug"], properties: { slug: { type: "string" } } } },
  { name: "export_board_definition", description: "Export a board as a portable JSON definition document: lanes, statuses, guards, triggers, transition policy, defaultWorkflows. Round-trips through import_board_definition.", inputSchema: { type: "object", required: ["boardSlug"], properties: { boardSlug: { type: "string" } } } },
  { name: "describe_board_transitions", description: "Describe a board's transition policy: every allowed cross-lane move, who can drive it (manual, workflow, run origin, actor id/role), and the message returned when a move is denied.", inputSchema: { type: "object", required: ["boardSlug"], properties: { boardSlug: { type: "string" } } } },
  { name: "check_board_transition", description: "Preflight a proposed move against a board's transition policy without mutating anything. Answers whether the caller (as a manual user, a specific workflow/run/agent) would be allowed.", inputSchema: { type: "object", required: ["boardSlug", "fromStatus", "toStatus"], properties: { boardSlug: { type: "string" }, fromStatus: { type: "string" }, toStatus: { type: "string" }, actorRole: { type: "string" }, workflowSlug: { type: "string" }, runOrigin: { type: "string" }, runId: { type: "string" } } } },
  { name: "validate_board_definition", description: "Validate a portable board definition document without importing it. Returns {valid, definition, preview: {laneCount, scheduleCount, transitions}} on success or {valid: false, error} on failure.", inputSchema: { type: "object", required: ["definition"], properties: { definition: { type: "object" } } } },
  { name: "import_board_definition", description: "Import a portable board definition. Creates the board when new, updates lanes/triggers/policy when an existing board shares the slug, and reconciles any embedded schedule hookups by slug so re-imports never duplicate cron jobs.", inputSchema: { type: "object", required: ["definition"], properties: { definition: { type: "object" }, slug: { type: "string", description: "Optional slug override (defaults to definition.slug) so an agent can provision the same template as several boards." } } } },
