...
// The landing page is rendered at request time so visitors see live workflow
// state, real health dots, and (when authed locally) deployment topology — no
// new template engine; we just substitute placeholders in landing.html.
const HTML_ENTITY_MAP = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" };
function escapeHtml(value) {
  return String(value == null ? "" : value).replace(/[&<>"']/g, (ch) => HTML_ENTITY_MAP[ch]);
}

function lastRunForCapability(slug) {
  if (!slug) return null;
  try {
    const recent = listRuns({ limit: 1, q: slug });
    const match = recent.find((row) => row.capabilitySlug === slug);
    return match || null;
  } catch {
    return null;
  }
...
function badgeForRunStatus(status) {
  switch ((status || "").toLowerCase()) {
    case "succeeded": return { cls: "badge badge-ok", label: "ok" };
    case "failed":
    case "error":
    case "cancelled":
    case "rejected":
      return { cls: "badge badge-failed", label: status };
    case "running":
    case "assigned":
      return { cls: "badge badge-running", label: "running" };
    case "queued":
    case "waiting_approval":
      return { cls: "badge badge-warn", label: status };
    default:
      return { cls: "badge badge-idle", label: status || "idle" };
  }
}
...
function renderWorkflowsPanel(workflows) {
  if (!workflows.length) {
    return [
      ...
      '  <p class="muted">Add one to <code>workflow-templates/workflows/</code> and register it — see <a href="/docs/quickstart">/docs/quickstart</a> or the project <a href="https://github.com/yolo-maxi/runyard#registering-a-workflow">README</a>.</p>',
      '  <p class="muted">Once a workflow is registered, this panel will show its name, last-run status, and a one-click Run button.</p>',
      '</div>'
    ].join("\n");
  }
  const rows = workflows.map((wf) => {
    const last = wf.lastRun;
    const badge = badgeForRunStatus(last ? last.status : "");
    const ts = last && (last.completedAt || last.startedAt || last.createdAt)
      ? escapeHtml(new Date(last.completedAt || last.startedAt || last.createdAt).toISOString())
      : '<span class="muted">never run</span>';
    const lastLink = last ? `<a href="/app#runs/${encodeURIComponent(last.id)}/logs">${escapeHtml(last.id)}</a>` : '<span class="muted">—</span>';
    return [
      '<tr>',
      `  <td><div class="wf-name">${escapeHtml(wf.name)}</div><div class="wf-slug">${escapeHtml(wf.slug)}</div></td>`,
      `  <td><span class="${badge.cls}">${escapeHtml(badge.label)}</span></td>`,
      `  <td>${ts}</td>`,
      `  <td>${lastLink}</td>`,
      `  <td style="text-align:right"><a class="button primary" href="/app#workflows/${encodeURIComponent(wf.slug)}/run">Run</a></td>`,
      '</tr>'
    ].join("\n");
  }).join("\n");
  return [
    ...
    '  <table>',
    '    <thead><tr><th>Name</th><th>Last status</th><th>Last run (UTC)</th><th>Last run id</th><th></th></tr></thead>',
    `    <tbody>${rows}</tbody>`,
    '  </table>',
    '</div>'
  ].join("\n");
...
// Try `systemctl is-active <unit>`. We swallow errors so a non-systemd host
// (dev laptop / container) gets a harmless "unknown" without crashing the page.
function systemctlIsActive(unit) {
  try {
    const out = execFileSync("systemctl", ["is-active", unit], { stdio: ["ignore", "pipe", "ignore"], timeout: 2000 });
    return String(out).trim();
  } catch (error) {
    if (error && error.stdout) return String(error.stdout).trim() || "unknown";
    return "unknown";
  }
}

function classifyStatus(state) {
  if (state === "active") return { cls: "status-dot-ok", label: "ok" };
  if (state === "activating" || state === "reloading") return { cls: "status-dot-warn", label: state };
  if (state === "inactive" || state === "failed" || state === "deactivating") return { cls: "status-dot-danger", label: state };
  return { cls: "status-dot-info", label: state || "unknown" };
}

function lastOptimizerRunAgeHours() {
  try {
    const runs = listRuns({ limit: 50 });
    const optimizer = runs.find((r) => /optimi[sz]e|improve|harden/i.test(`${r.capabilitySlug} ${r.capabilityName}`));
    if (!optimizer || !optimizer.completedAt) return null;
    return (Date.now() - Date.parse(optimizer.completedAt)) / 3_600_000;
  } catch {
    return null;
  }
}

function computeLandingStatuses() {
  // 1) Agents-run dot: the runner heartbeat (and systemd unit, when present).
  const runnerSvc = systemctlIsActive("smithers-hub-runner");
  let runnerLive = false;
  try { runnerLive = listRunners().some((r) => r.online); } catch { /* db may be unavailable */ }
  let agentsState;
  let agentsTooltip;
  if (runnerSvc === "active" || runnerLive) {
    agentsState = "active";
    agentsTooltip = `runner: ${runnerSvc === "active" ? "active" : "heartbeat ok"}`;
  } else if (runnerSvc === "unknown" && !runnerLive) {
    agentsState = "inactive";
    agentsTooltip = "runner: inactive";
  } else {
    agentsState = runnerSvc;
    agentsTooltip = `runner: ${runnerSvc}`;
  }
  const agents = { ...classifyStatus(agentsState), tooltip: agentsTooltip };
  // 2) Harden dot: workflow store reachability (DB read).
  let storeOk = false;
  let storeDetail = "store: unknown";
  try { listCapabilities(); storeOk = true; storeDetail = "workflow store: reachable"; }
  catch (error) { storeDetail = `workflow store: ${error && error.message ? error.message : "unreachable"}`; }
  const harden = storeOk
    ? { cls: "status-dot-ok", label: "ok", tooltip: storeDetail }
    : { cls: "status-dot-danger", label: "down", tooltip: storeDetail };
  // 3) Compile dot: last optimizer-style run age.
  const ageH = lastOptimizerRunAgeHours();
  let compile;
  if (ageH == null) {
    compile = { cls: "status-dot-info", label: "idle", tooltip: "optimizer: no completed run yet" };
  } else if (ageH < 36) {
    compile = { cls: "status-dot-ok", label: `last ${ageH.toFixed(1)}h ago`, tooltip: `optimizer: last run ${ageH.toFixed(1)}h ago` };
  } else if (ageH < 24 * 7) {
    compile = { cls: "status-dot-warn", label: `last ${ageH.toFixed(0)}h ago`, tooltip: `optimizer: stale (${ageH.toFixed(0)}h ago)` };
  } else {
    compile = { cls: "status-dot-danger", label: `last ${(ageH / 24).toFixed(0)}d ago`, tooltip: `optimizer: not run in ${(ageH / 24).toFixed(0)} days` };
  }
  return { agents, harden, compile };
}

// One real workflow definition we keep inline so the hero shows actual
// inputs, steps, and outputs rather than aspirational copy. Mirrors
// workflow-templates/workflows/hello.tsx in shape; kept in sync manually.
const HERO_WORKFLOW_EXAMPLE = `// workflow-templates/workflows/hello.tsx
...
<Workflow name="hello">
  <Task id="hello" output={helloOutput} agent={providers.claude}>
    Write a sentence about {ctx.input.topic}.
    Return JSON with "answer" and "wordCount".
  </Task>
</Workflow>`;
...
function readDeployManifest() {
  const candidates = [
    path.join(env.dataDir, "deploy.json"),
    path.join(env.root, "deploy.json"),
    path.join(env.root, "deploy", "deploy.json")
  ];
...
function renderOpsBlock(req) {
  // Local requests (loopback) or sessions with a valid cookie may see the
  // deployment topology. Everything else gets nothing — the block is silent
  // so an external visitor can't probe for it.
  const allowed = isLocalRequest(req) || Boolean(parseCookies(req)?.session);
  if (!allowed) return "";
  const manifest = readDeployManifest() || {};
  const runners = (() => { try { return listRunners(); } catch { return []; } })();
  ...
  const rows = [];
  rows.push({ label: "Hub host", value: manifest.hubHost || env.hostname || env.baseUrl, status: null });
  rows.push({ label: "Runner host(s)", value: manifest.runnerHost || runners.map((r) => r.hostname || r.name).filter(Boolean).join(", ") || "—", status: null });
  rows.push({ label: "Last deploy SHA", value: manifest.deploySha || manifest.sha || "—", status: null });
  for (const runner of runners.slice(0, 4)) {
    const age = ageSeconds(runner.lastHeartbeatAt);
    rows.push({
      label: `Runner ${escapeHtml(runner.name || runner.id)}`,
      value: age == null ? "no heartbeat" : `heartbeat ${age}s ago`,
      stale: age != null && age > 60
    });
  }
  const tbody = rows.map((row) => `<tr class="${row.stale ? "row-stale" : ""}"><th>${escapeHtml(row.label)}</th><td>${row.value}</td></tr>`).join("");
  return [
    '<section class="band">',
    '  <div class="band-inner">',
    '    <div class="toolbar"><h1>Deployment</h1><span class="muted">Visible to local sessions only</span></div>',
    `    <div class="ops-block"><table>${tbody}</table></div>`,
    '  </div>',
    '</section>'
  ].join("\n");
}

let landingTemplateCache = null;
function loadLandingTemplate() {
  if (landingTemplateCache && env.isProduction) return landingTemplateCache;
  landingTemplateCache = readFileSync(path.join(env.root, "public", "landing.html"), "utf8");
  return landingTemplateCache;
}

function renderLanding(req) {
  let workflows = [];
  try {
    workflows = listCapabilities().map((cap) => ({
      slug: cap.slug,
      name: cap.name,
      lastRun: lastRunForCapability(cap.slug)
    }));
  } catch { workflows = []; }
  const statuses = computeLandingStatuses();
  const workflowsForJs = workflows.map((w) => ({ slug: w.slug, name: w.name }));
  const workflowsJsonScript = `<script>window.__RUNYARD_WORKFLOWS__ = ${JSON.stringify(workflowsForJs).replace(/</g, "\\u003c")};</script>`;
  const replacements = {
    "{{WORKFLOWS_PANEL}}": renderWorkflowsPanel(workflows),
    "{{HERO_WORKFLOW_EXAMPLE}}": escapeHtml(HERO_WORKFLOW_EXAMPLE),
    "{{STATUS_DOT_AGENTS_CLASS}}": statuses.agents.cls,
    "{{STATUS_DOT_AGENTS_TOOLTIP}}": escapeHtml(statuses.agents.tooltip),
    "{{STATUS_DOT_AGENTS_LABEL}}": escapeHtml(statuses.agents.tooltip),
    "{{STATUS_DOT_HARDEN_CLASS}}": statuses.harden.cls,
    "{{STATUS_DOT_HARDEN_TOOLTIP}}": escapeHtml(statuses.harden.tooltip),
    "{{STATUS_DOT_HARDEN_LABEL}}": escapeHtml(statuses.harden.tooltip),
    "{{STATUS_DOT_COMPILE_CLASS}}": statuses.compile.cls,
    "{{STATUS_DOT_COMPILE_TOOLTIP}}": escapeHtml(statuses.compile.tooltip),
    "{{STATUS_DOT_COMPILE_LABEL}}": escapeHtml(statuses.compile.tooltip),
    "{{OPS_BLOCK}}": renderOpsBlock(req),
    "{{WORKFLOWS_JSON_SCRIPT}}": workflowsJsonScript
  };
  let html = loadLandingTemplate();
...
app.get("/", (req, res) => {
  try {
    res.type("html").send(renderLanding(req));
  } catch (error) {
    // If rendering fails for any reason (e.g. corrupt template) fall back to
    // the raw static file so the page still loads — we'd rather show stale
    // copy than serve a 500 on the root.
    res.sendFile(path.join(env.root, "public", "landing.html"));
  }
});
app.get("/ops", (req, res) => {
  if (!(isLocalRequest(req) || parseCookies(req)?.session)) {
