...
SMITHERS_HUB_DATA_DIR=$DATA_DIR
...
SMITHERS_HUB_URL=http://127.0.0.1:$PORT
...
install_units() {
  ...
  local hub_src="$HOME_DIR/deploy/smithers-hub.service"
  ...
  if [ "$INSTALL_RUNNER" = "1" ]; then
    local runner_src="$HOME_DIR/deploy/smithers-hub-runner.service"
  ...
  if [ "$INSTALL_RUNNER" = "1" ]; then
    ...
    # (and log an auth error) until the operator fills SMITHERS_HUB_TOKEN.
...
──────────────────────────────────────────────────────────────────────────────
 ...
 Next:
   ...
   2. Put it in $ENV_DIR/runner.env (SMITHERS_HUB_TOKEN=...), then:
