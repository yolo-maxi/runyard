...
describe("Hardening: scopes, tokens, run state, webhook, health", () => {
  ...
  it("enforces token scopes", async () => {
    ...
    const run = await raw("/api/capabilities/prepare-spec/run", { method: "POST", body: { input: { goal: "scoped" } } }, mcpToken);
  ...
  it("guards run state transitions", async () => {
    const created = await api("/api/capabilities/prepare-spec/run", { method: "POST", body: { input: { goal: "state" } } });
  ...
  it("does not double-assign a queued run to two runners", async () => {
    const created = await api("/api/capabilities/prepare-spec/run", { method: "POST", body: { input: { goal: "claim" } } });
    const a = await api("/api/runners/register", { method: "POST", body: { name: "a", hostname: "a", tags: ["node"] } });
    const b = await api("/api/runners/register", { method: "POST", body: { name: "b", hostname: "b", tags: ["node"] } });
  ...
  it("blocks runners from touching runs they do not own", async () => {
    const created = await api("/api/capabilities/prepare-spec/run", { method: "POST", body: { input: { goal: "owner" } } });
  ...
  it("does not let a runner token hijack another runner's id", async () => {
    ...
    const mine = await raw("/api/runners/register", { method: "POST", body: { name: "mine", tags: ["node"] } }, runnerToken);
    const myId = mine.data.runner.id;
    // A different token (admin) tries to register using my runner id -> must NOT overwrite mine.
    const other = await api("/api/runners/register", { method: "POST", body: { id: myId, name: "hijack", tags: ["node"] } });
